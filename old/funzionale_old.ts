...
describe("VC selective disclosure - performance report", async function () {
  it("misura tempi e gas delle principali operazioni del prototipo", async function () {
    ...
    const CRED_PATIENT = hash("vc:patient:bob:identity:v1");
    ...
    await measureTx(
      "authorize issuer Org1 for PatientIdentityVC",
      () => trustRegistry.authorizeIssuer(DID_ORG1, VC_PATIENT_IDENTITY),
      "trust",
    );
    await measureTx(
    ...
    async function issueCredential({
      ...
      pushRecord({
        ...
        operation: `build PresentedCredential ${label}`,
        kind: "view",
        ms: endedAt - startedAt,
        success: true,
        notes: "operazione off-chain: hashing disclosure + firma issuer",
      });
    ...
    const patientPresentedCredential = await issueCredential({
      credentialId: CRED_PATIENT,
      issuerSigner: organization1,
      issuerDID: DID_ORG1,
      subjectDID: DID_PATIENT,
      credentialType: VC_PATIENT_IDENTITY,
      label: "patient-bob-identity-v1",
      claims: {
        role: "Patient",
        patientCode: "PATIENT-BOB-001",
      },
    });
    ...
    await measureTx(
      "map PatientIdentityVC to Patient role",
      () => policyRegistry.setCredentialTypeRole(VC_PATIENT_IDENTITY, ROLE.Patient),
      "policy",
    );
    await measureTx(
    ...
    const auditorCanReadBeforeRestriction = await measureView(
      "canAccess Auditor before patient-driven restriction",
      () => accessController.canAccess(DID_AUDITOR, auditorPresentedCredential, documentId, ACTION_READ, PURPOSE_CARE),
      "access-control",
    );
    assert.equal(auditorCanReadBeforeRestriction, true);

    const policyBlock = await ethers.provider.getBlock("latest");
    const restrictionValidFrom = BigInt(policyBlock!.timestamp);
    const restrictionValidUntil = restrictionValidFrom + 7n * 24n * 60n * 60n;

    await measureTx(
      "register patient-driven restriction for Auditor READ",
      () =>
        patientDrivenPolicyRegistry
          ...
          .registerRestriction(
            ...
            hash("patient-policy:restrict-auditor-read:perf"),
          ),
      "patient-driven-policy",
    );

    const auditorRestricted = await measureView(
      "isRestricted Auditor READ",
      () => patientDrivenPolicyRegistry.isRestricted(DID_AUDITOR, documentId, ACTION_READ, PURPOSE_CARE),
      "patient-driven-policy",
    );
    assert.equal(auditorRestricted, true);

    const auditorCanReadAfterRestriction = await measureView(
      "canAccess Auditor after patient-driven restriction",
      () => accessController.canAccess(DID_AUDITOR, auditorPresentedCredential, documentId, ACTION_READ, PURPOSE_CARE),
      "access-control",
    );
    assert.equal(auditorCanReadAfterRestriction, false);
